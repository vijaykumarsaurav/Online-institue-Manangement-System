...
<head>
<title BGCOLOR="RED">emp salary paid</title>
<script type ="text/javascript" src="javascript.js"></script>
</head>
 <form action="http://localhost:8000/oims/emp_salary_save.jsp" method="POST" onsubmit="return empmakecheck()">
<center>
<fieldset style="width:500;background:rgb(251,252,188); text-align:center;"><br/>
<h2 style="color:rgb(234,9,48);text-align:center;"> Employee Salary Payment </h2>
<hr>
...
try
{
	...
	ResultSet rsc=stmt.executeQuery("select COUNT(emp_id) from employee where emp_id='"+emp_id+"'");
	Boolean row=rsc.next();
  DateFormat df1 = new SimpleDateFormat("yyyy-MM-dd");
  String dat = df1.format(new java.util.Date());
  if(rsc.getInt(1)==1)
  {	
	ResultSet rs=stmt.executeQuery("select name,father_name,post,salary,dat from employee where emp_id='"+emp_id+"'");
	while(rs.next())
	{
		course=rs.getString(3);
		out.println("<tr><td><b> Employee ID </b></td><td>"+emp_id+" </td><input type='hidden' name='emp_id' value='"+emp_id+"'></tr>");
		...
		out.println("<tr><td><b> Post </b></td><td>"+rs.getString(3)+" </td><input type='hidden' name='course' value='"+rs.getString(3)+"'></tr>");
		out.println("<tr><td><b> Salary  </b></td><td>"+rs.getString(4)+" </td><input type='hidden' name='batch' value='"+rs.getString(4)+"'></tr>");
		out.println("<tr><td><b> Date  </b></td><td>"+rs.getString(5)+" </td><input type='hidden' name='batch' value='"+rs.getString(5)+"'></tr>");

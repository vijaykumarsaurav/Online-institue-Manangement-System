...
%>
<%	try{
ResultSet rs0=stmt.executeQuery("select COUNT(form_no) from student");
...
while(rs.next())
{
					...
					out.println("<tr bgcolor='darer 25%'><td>"+c+"</td><td> <a href='student_display_form_wise.jsp?form_no="+rs.getString(1)+"'>"+rs.getString(1)+"</a></td><td>"+rs.getString(2)+"</td><td>"+rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getString(6)+"</td><td>"+rs.getString(7)+"</td></tr>");
					flag=1;
					}	
					else
					{ 				
				out.println("<tr bgcolor='white'><td>"+c+"</td><td> <a  href='student_display_form_wise.jsp?form_no="+rs.getString(1)+"'>"+rs.getString(1)+"</a></td><td>"+rs.getString(2)+"</td><td>"+rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getString(6)+"</td><td>"+rs.getString(7)+"</td></tr>");
					flag=0;					
					}
					c=c+1;
}
ResultSet rs1=stmt.executeQuery("select sum(registation)  from payment");
rs1.next();
int reg=rs1.getInt(1);
ResultSet rs2=stmt.executeQuery("select sum(paid) from payment");
rs2.next();
int paid=rs2.getInt(1);
ResultSet rs3=stmt.executeQuery("select sum(credit)  from payment");
rs3.next();
int credit=rs3.getInt(1);
ResultSet rs4=stmt.executeQuery("select sum(total) from payment");
rs4.next();
